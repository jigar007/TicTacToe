...
public class PlayerManager {
        ...
        for (int i = 0; i < objInPlayer; i++) {
            if (playerList[i] != null) {
                if (username.equals(playerList[i].getUserName())) {
                    //Return index of array if player is exist
                    return i;
                }
            }
        }
        // Return -1 is player not exist
        return -1;
    }
    ...
    public void addPlayer(String userName, String familyName, String givenName) {
        ...
        if (checkUserName(userName) == -1) {
            // If player with same username is not exist then adds to array
    ...
    public void removePlayer(String userName) {
        ...
        if (index != -1) {
            // Shift array left side by one index
