...
public class PlayerManager {
    ...
    public void displayPlayer(String displayName) {
        if (checkUserName(displayName) != -1) {
            player = (Player) playerList[checkUserName(displayName)];
...
//        return null;
    ...
    /**
     * Load all data from file to current system
     */
    public void getDataFromFile() throws IOException, ClassNotFoundException {
        if (!dataFile.exists()) {
            ...
            playerList = (Player[]) ois.readObject();
            while (playerList[objInPlayer] != null) {
                objInPlayer++;
            }
            ois.close();
            fin.close();
